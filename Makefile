...
pugixmlbuild :
	...
	# Pugixml build generates the build files in a directory with a name generated by the compiler
	# In our case, instead of something like "gcc+", it is the full path of em++ : "/emscripten/em++" when run inside Docker
	# TODO : improve this by modifying the Makefile of pugixmlbuild, to be compatible with any emscripten location
	cd pugixml-1.9; emmake make build/make-/emscripten/em++-debug-standard-c++11/src/pugixml.cpp.o
	mkdir -p pugixmlbuild/lib pugixmlbuild/include
	cp pugixml-1.9/build/make-/emscripten/em++-debug-standard-c++11/src/pugixml.cpp.o pugixmlbuild/build
